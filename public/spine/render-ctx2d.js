...
class RenderCtx2D_Resource {
  constructor(images, skin_info_map) {
    this.images = images;
    this.skin_info_map = skin_info_map;
  }
}

function RenderCtx2D() {
}
...
RenderCtx2D.prototype.loadData = function(spine_data, atlas_data, images) {
  ...
  return new RenderCtx2D_Resource(render.images, render.skin_info_map);
}

/**
 * @param {RenderCtx2D_Resource} resource
 */
RenderCtx2D.prototype.setResource = function (resource) {
  var render = this;
  
  render.images = resource.images;
 ...
 * @param {atlas.Data} atlas_data
 */
RenderCtx2D.prototype.drawDebugPose = function(spine_pose, atlas_data, flags) {
  var render = this;
  var ctx = render.ctx;
  var default_skin_info = render.skin_info_map['default'];
 ...
 * @param {atlas.Data} atlas_data
 */
RenderCtx2D.prototype.drawDebugData = function(spine_pose, atlas_data, flags) {
  var render = this;
  var ctx = render.ctx;
  var default_skin_info = render.skin_info_map['default'];
  var stroke_style = 'rgba(255,255,255,1.0)';
...
function ctxDrawImageMesh(ctx, triangles, positions, texcoords, image, site, page) {
  ...
  for (var index = 0; index < triangles.length;) {
    ...
    ctx.lineTo(x1, y1);
    ctx.lineTo(x2, y2);
    // ctx.moveTo(Math.trunc(x0 + 0.5), Math.trunc(y0 + 0.5));
    // ctx.lineTo(Math.trunc(x1 + 0.5), Math.trunc(y1 + 0.5));
    // ctx.lineTo(Math.trunc(x2 + 0.5), Math.trunc(y2 + 0.5));
    ctx.closePath();
    ctx.clip();
    x1 -= x0;
    y1 -= y0;
